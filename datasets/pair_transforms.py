...
class ToTensor(nn.Module):
    ...
    def forward(self, inputs):
        for k in list(inputs):
            if "color" in k or "color_weather" in k or "color_contrast" in k:
...
class RandomResizeCrop(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 对雨天图像进行插值到晴天图像的大小，方便后续的随机裁剪
        for k in list(inputs):
            if "color_weather" in k or "color_contrast" in k:
                inputs[k] = F.interpolate(inputs[k][None, ...], size=(H, W), mode="bicubic", align_corners=True)[0].clamp(min=0., max=1.)
        ...
        grid = meshgrid(torch.linspace(-1, 1, int(W * factor)), torch.linspace(-1, 1, int(H * factor)), indexing="xy")
        ...
        for k in list(inputs):
            if "color" in k or "color_weather" in k or "color_contrast" in k:
                ...
                if "color_weather" in k:
                    inputs[("color_aug", im)] = input
                elif "color_contrast" in k:
                    inputs[("color_cst", im)] = input
                elif "color" in k:
                    inputs[(n, im)] = input
                    if inputs["save_mode"]:
                        inputs[("color_cst", im)] = input.clone()
...
class Resize(nn.Module):
    ...
    def __init__(self, target_size=(192, 640)):
        super().__init__()
        self.target_size = target_size
        grid = meshgrid(torch.linspace(-1, 1, target_size[1]), torch.linspace(-1, 1, target_size[0]), indexing="xy")
        self.grid = torch.stack(grid, dim=0)

    def forward(self, inputs):
        _, H, W = inputs[("color", "l", -1)].shape
        ...
        for k in list(inputs):
            if "color" in k or "color_weather" in k or "color_contrast" in k:
                ...
                input = input.clamp(min=0., max=1.)
                if "color_weather" in k:
                    inputs[("color_aug", im)] = input.clone()
                elif "color_contrast" in k:
                    inputs[("color_cst", im)] = input.clone()
                else:
                    inputs[(n, im)] = input
...
class RandomGamma(nn.Module):
    ...
    def forward(self, inputs):
        if random.random() < self.p:
            ...
            for k in list(inputs):
                if "color_aug" in k or "color_cst" in k:
